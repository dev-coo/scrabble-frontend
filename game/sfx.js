...
window.Sfx = (function () {
  ...
  var api = {
    ...
    /* 턴이 넘어갔을 때 — "띵—" 하고 길게 남는 종소리.

       처음에는 사인파만 겹쳐 만들었는데 너무 얌전해서 다른 소리에
       묻혔습니다. 종이 멀리서도 들리는 이유는 세 가지입니다.
         ① 때리는 순간이 아주 짧고 세다
         ② 배음이 정확한 정수배가 아니라 살짝 어긋나 있다
            (그래서 "웅—" 하고 울리는 맛이 납니다)
         ③ 소리가 길게 남는다
       그 셋을 다 넣고 소리도 두 배 가까이 키웠습니다.
    ...
    ding: function (left) {
      ...
      var base = [523.25, 622.25, 739.99][Math.max(0, Math.min(2, left == null ? 2 : left))];

      // ① 때리는 순간 — 짧고 단단하게. 이게 있어야 "쳤다"로 들립니다.
      knock(c, t, base * 5, 0.16, 0.022, 11);
      knock(c, t, base * 2.4, 0.10, 0.035, 9);

      // ② 종의 몸통. 배음을 일부러 조금씩 어긋나게 얹습니다.
      tone(c, t, base,        0.24, 1.6, 'triangle');
      tone(c, t, base * 2.01, 0.11, 1.3, 'sine');
      tone(c, t, base * 2.76, 0.06, 0.9, 'sine');   // 정수배가 아닌 음
      tone(c, t, base * 4.07, 0.035, 0.6, 'sine');

      // ③ 아주 살짝 늦게 한 겹 더 — 소리가 넓어집니다.
      tone(c, t + 0.012, base * 1.005, 0.10, 1.5, 'triangle');
    },
